...
{
    class cadProfesiones
    {

        private ConexionBaseDatos conn;
        public cadProfesiones()
        {
            conn = new ConexionBaseDatos();
        }
        public Boolean insert(Profesiones bite)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "INSERT INTO catalogo_profesiones (nombre) values (@1)";
        ...
        public Boolean update(Profesiones bite)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "update catalogo_profesiones set nombre=@1  where codigo=@4";
                SqlCommand comando = new SqlCommand(sql, conn.Conectar());
                comando.Parameters.AddWithValue("@1", bite.Nombres);
        ...
        public Boolean delete(Profesiones bite)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "delete from catalogo_profesiones  where codigo=@4";
        ...
        public DataTable getAll()
        {
            ...
            try
            {
                String sql = "select codigo, nombre  from catalogo_profesiones ";
                SqlCommand comando = new SqlCommand(sql, conn.Conectar());
        ...
        public DataTable busqueda(String val)
        {
            ...
            try
            {
                String sql = " select  codigo ,nombre  from catalogo_profesiones where nombre LIKE CONCAT('%',@1,'%')" +
