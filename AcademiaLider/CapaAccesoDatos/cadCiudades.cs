...
namespace AcademiaLider.CapaAccesoDatos
{
    class cadCiudades
    {
        ...
        public Boolean insert(Ciudades city)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "INSERT INTO catalogo_ciudades (nombre) values (@1)";

                SqlCommand comando = new SqlCommand(sql, conn.Conectar());
                comando.Parameters.AddWithValue("@1", city.Nombres);
        ...
        public Boolean update(Ciudades city)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "update catalogo_ciudades set nombre=@1  where codigo=@4";

                SqlCommand comando = new SqlCommand(sql, conn.Conectar());

                comando.Parameters.AddWithValue("@1", city.Nombres);
                comando.Parameters.AddWithValue("@4", city.Codigo);
                fila = comando.ExecuteNonQuery();
                conn.Cerrar();
                if (fila > 0)
                {
        ...
        public Boolean delete(Ciudades city)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "delete from catalogo_ciudades  where codigo=@4";
        ...
        public DataTable getAll()
        {
            ...
            try
            {
                String sql = "select codigo, nombre  from catalogo_ciudades ";
                SqlCommand comando = new SqlCommand(sql, conn.Conectar());
        ...
        public DataTable busqueda(String val)
        {
            ...
            try
            {
                String sql = " select  codigo ,nombre  from catalogo_ciudades  where nombre LIKE CONCAT('%',@1,'%')" + 
