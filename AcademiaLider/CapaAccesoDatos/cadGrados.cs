...
namespace AcademiaLider.CapaAccesoDatos
{
    class cadGrados
    {
        ...
        public Boolean insert(Grados bite)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "INSERT INTO catalogo_grados_academicos (nombre) values (@1)";
        ...
        public Boolean update(Grados bite)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "update catalogo_grados_academicos set nombre=@1  where codigo=@4";
                SqlCommand comando = new SqlCommand(sql, conn.Conectar());
                comando.Parameters.AddWithValue("@1", bite.Nombres);
        ...
        public Boolean delete(Grados bite)
        {
            Boolean val = false;
            int fila = -1;
            try
            {
                String sql = "delete from catalogo_grados_academicos  where codigo=@4";
        ...
        public DataTable getAll()
        {
            ...
            try
            {
                String sql = "select codigo, nombre  from catalogo_grados_academicos ";
                SqlCommand comando = new SqlCommand(sql, conn.Conectar());
        ...
        public DataTable busqueda(String val)
        {
            ...
            try
            {
                String sql = " select  codigo ,nombre  from catalogo_grados_academicos where nombre LIKE CONCAT('%',@1,'%')" +
